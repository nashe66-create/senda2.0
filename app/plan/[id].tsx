...
export default function PlanDetailScreen() {
  ...
  const handleAddCommitment = async () => {
    ...
    try {
      const rate = await fetchLiveFxRate('GBP', destinationCurrency);
      const amountDestination = rate ? amount * rate : 0;
  ...
  const handleConfirmPlan = () => {
    ...
    Alert.alert(
      'Confirm Plan',
      `Lock in ${formatGBP(Number(plan.total_gbp))} for ${plan.commitments.length} recipient${plan.commitments.length !== 1 ? 's' : ''}? You can send the transfers when ready.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Confirm Plan',
          onPress: async () => {
            try {
              ...
              await createTransaction(id, Number(plan.total_gbp));
              await loadPlan();
            } catch (e: any) {
  ...
  const handleSendPlan = () => {
    ...
    Alert.alert(
      'Send Transfers',
      `Send ${formatGBP(Number(plan.total_gbp))} to ${plan.commitments.length} recipient${plan.commitments.length !== 1 ? 's' : ''} via Flutterwave now?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Send Now',
          onPress: async () => {
            ...
            setSendResult(null);
            try {
              const result = await sendPlanTransfers(id);
              ...
              if (result.failed === 0) {
                ...
                await updatePlan(id, { status: 'failed' });
              }
              await loadPlan();
            } catch (e: any) {
              Alert.alert('Error', e.message || 'Failed to send transfers');
            } finally {
              setSending(false);
            }
          },
      ...
      >
        ...
        {sendResult && (
          <View style={styles.sendResultCard}>
            <Text style={styles.sendResultTitle}>Transfer Results</Text>
        ...
        {plan.status === 'confirmed' && (
          ...
          >
            ...
            {'  '}Send {formatGBP(Number(plan.total_gbp))}
          </Button>
        )}
