...
export default function SettingsScreen() {
  ...
  const KycIcon = kycConfig[kycStatus].icon;
  ...
  const handleStartKyc = () => {
    Alert.alert(
      'KYC Verification',
      'Identity verification is handled by Flutterwave. In production, this would launch the Flutterwave KYC flow to collect and verify your documents (ID, selfie, address proof). For this demo, you can simulate verification.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            await supabase
              .from('profiles')
              .update({ kyc_status: 'verified', flutterwave_customer_id: 'flw_cust_demo_' + Date.now() })
              .eq('id', user?.id);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Card style={styles.kycCard}>
        <View style={[styles.kycIconWrap, { backgroundColor: kycConfig[kycStatus].bg }]}>
          <KycIcon color={kycConfig[kycStatus].color} size={24} strokeWidth={2} />
        </View>
        <View style={styles.kycInfo}>
          ...
          <View style={styles.kycStatusRow}>
            <View
              style={[
                styles.kycBadge,
                { backgroundColor: kycConfig[kycStatus].bg },
              ]}
            >
              <Text style={[styles.kycBadgeText, { color: kycConfig[kycStatus].color }]}>
                {kycConfig[kycStatus].label}
              </Text>
            </View>
          </View>
          <Text style={styles.kycDesc}>{kycConfig[kycStatus].desc}</Text>
        </View>
      </Card>
